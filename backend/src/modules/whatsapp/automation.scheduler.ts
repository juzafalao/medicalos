// ============================================================
// CRON JOBS - Automação de disparos WhatsApp (tenant-safe)
// Todas as queries operam per-tenant para garantir isolamento RLS
// ============================================================
...
export class AutomationScheduler {
  ...
  async sendConfirmations() {
    ...
    for (const tenant of tenants) {
      const appointments = await this.db.queryWithTenant(
        tenant.id,
      ...
      for (const appt of appointments) {
        await this.whatsapp.sendConfirmation(tenant.id, appt.id);
        totalSent++;
      }
    }

    this.logger.log(`Confirmações enviadas: ${totalSent}`);
  }
  ...
  async sendReminders() {
    ...
    for (const tenant of tenants) {
      const appointments = await this.db.queryWithTenant(
        ...
        [tenant.id, windowStart.toISOString(), windowEnd.toISOString()],
      );
      ...
      for (const appt of appointments) {
        ...
        await this.whatsapp.queueMessage(tenant.id, {
    ...
    this.logger.log(`Lembretes enviados: ${totalSent}`);
  }
  ...
  async markNoShows() {
    ...
    for (const tenant of tenants) {
      await this.db.queryWithTenant(
        ...
        [tenant.id, oneHourAgo.toISOString()],
      );
    }
  }
    ...
    for (const tenant of tenants) {
      await this.db.queryWithTenant(
        ...
        `UPDATE patients p SET last_appointment_at = (
           ...
           WHERE a.patient_id = p.id AND a.tenant_id = p.tenant_id AND a.status = 'completed'
         )
         WHERE p.tenant_id = $1
           AND EXISTS (
